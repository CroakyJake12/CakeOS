...
/// <summary>
/// Generic HUI preview/demo surface. Renders an HUI scene (the built-in demo
/// or a caller-supplied application root such as a Boards session scene)
/// through the shared layout/renderer/input router. Canvas has its own
/// application shell (<c>CakeOS.Canvas.App</c>) and no longer lives here.
/// </summary>
public sealed class HuiPreviewSurface : Control, IHavenMeasureContext, IDisposable
{
            ...
            foreach (var command in _renderer.Render(_root))
            {
                switch (command)
                {
                    ...
                    case HavenGlowCommand glow:
                        ...
                        break;
                    default:
                        throw new NotSupportedException($"Preview backend does not yet support {command.GetType().Name}.");
                }
            }
    ...
    private static (HuiPage Root, HuiButton Action, HuiText Status) BuildScene()
    {
        ...
        var title = new HuiText { Content = "A real HUI scene, rendered as a Windows desktop window." };
